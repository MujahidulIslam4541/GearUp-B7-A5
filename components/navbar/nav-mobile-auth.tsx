...
export function NavMobileAuth({ user, onClose }: NavMobileAuthProps) {
  ...
  if (user && user.name) {
    const dashboardHref = getDashboardRouteForRole(user.role || "user")
      ...
      <>
        ...
        <DropdownMenuItem className="cursor-pointer rounded-lg p-0">
          ...
          >
            ...
            <span>
              Dashboard ({user.role ? user.role.toLowerCase() : "user"})
            </span>
          </Link>
        </DropdownMenuItem>
        <DropdownMenuItem
